...
describe("apiRequest — guarda de concorrência do refresh", () => {
  beforeEach(() => {
    localStorage.clear();
    authSession.set({ accessToken: "token-velho", role: "ADMIN" });
  ...
  it("N chamadas concorrentes com 401 disparam só UMA chamada real a /auth/staff/refresh", async () => {
    let refreshCalls = 0;
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/auth/staff/refresh")) {
        refreshCalls++;
        return new Response(
          JSON.stringify({ accessToken: "token-novo", refreshToken: "refresh-novo", role: "ADMIN" }),
    ...
    const results = await Promise.all([
      apiRequest<{ ok: boolean }>("/agenda"),
      apiRequest<{ ok: boolean }>("/barbers"),
      apiRequest<{ ok: boolean }>("/services"),
      apiRequest<{ ok: boolean }>("/clients"),
    ]);

    expect(refreshCalls).toBe(1);
    expect(results).toEqual([
      { ok: true, path: expect.any(String) },
      { ok: true, path: expect.any(String) },
      { ok: true, path: expect.any(String) },
      { ok: true, path: expect.any(String) },
    ]);
  ...
  it("segunda tentativa também tomando 401 não entra em loop — o erro sobe de verdade", async () => {
    ...
    await expect(apiRequest("/agenda")).rejects.toMatchObject({ status: 401 });
