...
## 3. Importar e configurar o workflow no n8n

1. Abra `http://localhost:5678`, crie a conta de admin do n8n (primeiro acesso).
2. **Workflows → Import from File** → `workflows/prisma-notification-due.json`.
3. O workflow usa `$env.PRISMA_WEBHOOK_SECRET`, `$env.PRISMA_API_URL`,
   `$env.PRISMA_API_KEY`, `$env.EVOLUTION_URL`, `$env.EVOLUTION_API_KEY`,
...
4. `PRISMA_API_KEY` precisa ser uma API key de verdade, criada uma vez:
   ```bash
   curl -X POST http://localhost:3000/api-keys -H "Authorization: Bearer $ADMIN_TOKEN" \
     -H 'Content-Type: application/json' \
     -d '{"name":"n8n","scopes":["notifications:write"]}'
   ```
   Guarde o `key` (só aparece uma vez) em `PRISMA_API_KEY` no `.env`.
5. Abra o nó **"Recebe evento"** e confirme que **Raw Body está marcado** em Options —
   é o passo mais fácil de esquecer e o que mais silenciosamente quebra tudo (a
...
Automatizado (não toca em WhatsApp de propósito — ver `tests/integration/outbox-delivery.test.ts`
e `tests/integration/whatsapp-notification.test.ts`): já roda na suíte da API.
...
2. **Entrega real.** Force uma notificação a vencer (`UPDATE notifications SET
   ...
   incluídos.
3. **O timeout de 10s sob latência real.** Depois do envio, confira via psql:
   ```sql
...
4. **O campo do id da mensagem.** Abra a execução do nó "Envia via Evolution" no n8n e
   veja o corpo de resposta de verdade — se não for `key.id`, ajuste o nó "Reporta
