...
export async function productsRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { id: string } }>("/products/:id/images", async (request, reply) => {
  ...
  app.delete<{ Params: { id: string; imageId: string } }>("/products/:id/images/:imageId", async (request, reply) => {
    await removeProductImage(request.params.id, request.params.imageId);
    reply.code(204).send();
  });

  app.post<{ Params: { id: string } }>("/products/:id/stock-movements", async (request, reply) => {
  ...
  app.get<{ Params: { id: string } }>("/products/:id/stock-movements", async (request) => {
